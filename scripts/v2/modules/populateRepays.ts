...
export class PopulateRepays {
	...
	*/
    ...
    async getRepays(): Promise<Repays> {
        ...
        const repayDirectBN = BigInt2BN(
            repayDirect,
            this.trade.tokenIn.decimals
        )
        const directRepayLoanPoolInTokenOutBN = repayDirectBN.multipliedBy(
            BN(this.trade.loanPool.priceOut)
        )
        const directRepayLoanPoolInTokenOut = BN2BigInt(
            directRepayLoanPoolInTokenOutBN,
        ...
        const tradeSizeInTermsOfTokenOutOnLoanPool = ts.multipliedBy(
            BN(this.trade.loanPool.priceOut)
        )

        const simpleBN =
            tradeSizeInTermsOfTokenOutOnLoanPool.multipliedBy(1.003) // 0.3% fee
